...
# Import and configure the Google Gemini library
...
# --- 2. Pydantic Schemas (Our Data Contracts) ---
...
# --- 4. Document Processing and RAG Classes ---
...
processor = None  # Initialize as None, will be set in startup_event
...
async def hackrx_run(request: HackRxRequest, api_key: str = Depends(verify_api_key)):
    ...
    result = await loop.run_in_executor(None, processor.process_request, request)
...
def read_root():
    return {
        ...
        "embedding_dimension": processor.embedding_dim if processor else "Not initialized"
    }
